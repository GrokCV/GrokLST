...
class NormalizeHeiheLSTData(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        # see LoadHeiheLSTMatFile for details about results['ori_band_name_to_idx_dict'].
...
class NormalizeOpticalSARData(BaseTransform):
    ...
    def __init__(
        ...
        self.key = key
        assert norm_flag in ['zscore', 'minmax', None], "norm_flag should be in ['zscore', 'minmax', None]!"
        assert norm_flag == 'zscore' and (mean is not None) and (std is not None), "when norm_flag is 'zscore', you should provide 'mean' and 'std'! "
        assert norm_flag == 'minmax' and (min is not None) and (max is not None), "when norm_flag is 'minmax', you should provide 'min' and 'max'! "
        self.norm_flag = norm_flag
        self.mean = mean
        self.std = std
        self.min = min
    ...
    def transform(self, results: dict) -> dict:
        ...
        if isinstance(self.mean, float):
            ...
            raise ValueError("mean should be a float or a list with the correct number of values")
        ...
        if isinstance(self.std, float):
            ...
            raise ValueError("std should be a float or a list with the correct number of values")
        ...
        if self.norm_flag == "zscore":
            ...
            if isinstance(self.max, float):
                ...
                std_values = self.max
            # z-score
            data = (data - np.array(self.min)) / (np.array(self.max) - np.array(self.min))
