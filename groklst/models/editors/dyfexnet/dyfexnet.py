...
class DyFeXNet(BaseModule):
    def __init__(
        ...
        self.bridges = nn.ModuleList(
            [eval(module)(lst_dim=(i + 1) * num_feats, gui_dim=num_feats, scale=scale) for i in range(self.num_stages)]
        )
    ...
    def forward(self, lst, hr_guidance, gui_mask=None, **kwargs):
        ...
        if self.stem_use_dmlp:
            ...
            weights = weights.reshape(self.num_feats, self.gui_channels, self.kernel_size, self.kernel_size)
...
if __name__ == "__main__":
    ...
    net = DyFeXNet(in_channels=10, num_feats=32, kernel_size=3, scale=2).cuda()
