...
from .common import ConvBNReLU2D, invPixelShuffle, InvUpSampler, get_act, UpSampler
...
class FeatureInitialization(nn.Module):
    ...
    def __init__(self, num_features, scale, guidance_channel=1):
        ...
        self.depth_in = nn.Sequential(
            nn.Conv2d(in_channels=1, out_channels=num_features, kernel_size=3, stride=1, padding=1), nn.PReLU()
        )
...
class Compress(nn.Module):
    def __init__(self, num_features, act, norm, fuse_way="add"):
        ...
        if self.fuse_way == "cat":
            self.compress_out = ConvBNReLU2D(
                in_channels=2 * num_features, out_channels=num_features, kernel_size=1, padding=0, act=act
            )
    ...
    def __init__(self, num_features, act, norm):
        ...
        self.layers = nn.Sequential(
            *[
                ...
                ConvBNReLU2D(
                    in_channels=num_features, out_channels=num_features, kernel_size=3, stride=1, padding=1, norm=norm
                ),
            ]
        )
...
def variance_pool(x):
    ...
    return (
        (x - my_mean).pow(2).mean(dim=3, keepdim=False).mean(dim=2, keepdim=False).view(x.size()[0], x.size()[1], 1, 1)
    )
...
class GateConv2D(nn.Module):
    def __init__(self, num_features):
        ...
        self.Attention = nn.Sequential(
            nn.Conv2d(in_channels=num_features, out_channels=num_features, kernel_size=3, padding=1), nn.Sigmoid()
        )
        self.Feature = nn.Sequential(
            nn.Conv2d(in_channels=num_features, out_channels=num_features, kernel_size=3, padding=1), nn.PReLU()
        )
...
class MMAB(nn.Module):
    def __init__(self, num_features, reduction_ratio=4):
        ...
        self.squeeze = ConvBNReLU2D(
            in_channels=num_features * 2,
            out_channels=num_features * 2 // reduction_ratio,
            kernel_size=3,
            act="PReLU",
            padding=1,
        )

        self.excitation1 = ConvBNReLU2D(
            in_channels=num_features * 2 // reduction_ratio, out_channels=num_features, kernel_size=1, act="Sigmoid"
        )
        self.excitation2 = ConvBNReLU2D(
            in_channels=num_features * 2 // reduction_ratio, out_channels=num_features, kernel_size=1, act="Sigmoid"
        )
...
class AHMF(BaseModule):
    """
    title: High-Resolution Depth Maps Imaging via Attention-Based Hierarchical Multi-Modal Fusion
    paper: https://ieeexplore.ieee.org/document/9642435 or https://arxiv.org/pdf/2104.01530v2.pdf
    code: https://github.com/zhwzhong/AHMF
    """
    ...
    def __init__(
        ...
        self.head = FeatureInitialization(num_features=64, scale=upscale_factor, guidance_channel=guidance_in_channels)
        ...
        for _ in range(3):
            self.rgb_conv.append(ConvBNReLU2D(in_channels=64, out_channels=64, kernel_size=3, padding=1, act=self.act))
        ...
        for _ in range(4):
            self.fuse_conv.append(FuseNet(num_features=64, reduction_ratio=4, act=self.act, norm=None))

            self.compress_out.append(Compress(num_features=64, act=self.act, norm=None))
        ...
        self.up_conv = nn.Sequential(
            ConvBNReLU2D(in_channels=64 * 4, out_channels=64, kernel_size=1, padding=0, act=self.act),
            *UpSampler(scale=upscale_factor, n_feats=64),
    ...
    def forward(self, x, hr_guidance, **kwargs):
        ...
        depth_feature, guide_feature, _ = self.head(x, hr_guidance)
        ...
        fuse_feature = []
        for i in range(4):
            tmp = self.fuse_conv[i](depth=depth_out[3 - i], guide=guide_out[3 - i])
