...
class DyFeXNetInterlace(BaseModule):
    def __init__(
        self,
        in_channels: int = 1,
        gui_channels: int = 10,
        ...
        self.bridges = nn.ModuleList(
            [eval(module)(lst_dim=(i + 1) * num_feats, gui_dim=num_feats, scale=scale, channel_flag=flags[i]) for i in range(self.num_stages)]
        )
...
if __name__ == "__main__":
    ...
    net = DyFeXNetInterlace(in_channels=10, num_feats=32, kernel_size=3, scale=2).cuda()
