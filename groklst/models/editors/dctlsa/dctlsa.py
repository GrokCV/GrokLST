...
def sequential(*args):
    if len(args) == 1:
        if isinstance(args[0], OrderedDict):
            raise NotImplementedError("sequential does not support OrderedDict x.")
        return args[0]
    modules = []
    for module in args:
...
class LocalModule(nn.Sequential):
    def __init__(self, channels):
        ...
        self.add_module(
            "depthwise_conv", nn.Conv2d(channels, channels, padding=1, kernel_size=3, groups=channels, bias=False)
        )
...
class DCTLSA(BaseModule):
    """ 
    title: Densely Connected Transformer With Linear Self-Attention for Lightweight Image Super-Resolution
    paper: https://ieeexplore.ieee.org/document/10215496 
    
    code: https://github.com/zengkun301/DCTLSA
    """
    def __init__(self, 
                 in_nc=1, 
                 out_nc=1, 
...
if __name__ == "__main__":
    ...
    model = DCTLSA(upscale=8, in_nc=1, out_nc=1)
