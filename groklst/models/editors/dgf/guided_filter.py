...
class ConvGuidedFilter(nn.Module):
    def __init__(self, in_channels, num_features, radius=1, norm=nn.BatchNorm2d):
        super(ConvGuidedFilter, self).__init__()
        self.r = radius
        self.box_filter = nn.Conv2d(
            in_channels, in_channels, kernel_size=3, padding=radius, dilation=radius, bias=False
        )
        self.conv_a = nn.Sequential(
            nn.Conv2d(2 * in_channels, num_features, kernel_size=1, bias=False),
            norm(num_features),
            nn.ReLU(inplace=True),
            nn.Conv2d(num_features, num_features, kernel_size=1, bias=False),
            norm(num_features),
            nn.ReLU(inplace=True),
            nn.Conv2d(num_features, in_channels, kernel_size=1, bias=False),
        )
