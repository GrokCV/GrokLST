...
class Coupled_Layer(nn.Module):
    def __init__(self,
        ...
        self.bias_shared_1 = nn.Parameter(
            (torch.zeros(size=[self.coupled_number])))
        self.bias_depth_1 = nn.Parameter(
            (torch.zeros(size=[self.n_feats - self.coupled_number])))
        self.bias_rgb_1 = nn.Parameter(
            (torch.zeros(size=[self.n_feats - self.coupled_number])))

        self.bias_shared_2 = nn.Parameter(
            (torch.zeros(size=[self.coupled_number])))
        self.bias_depth_2 = nn.Parameter(
            (torch.zeros(size=[self.n_feats - self.coupled_number])))
        self.bias_rgb_2 = nn.Parameter(
            (torch.zeros(size=[self.n_feats - self.coupled_number])))
    ...
    def forward(self, feat_dlr, feat_rgb):
        ...
        feat_dlr = F.conv2d(feat_dlr,
                            torch.cat(
                                [self.kernel_shared_1, self.kernel_depth_1], dim=0),
                            torch.cat(
                                [self.bias_shared_1, self.bias_depth_1], dim=0),
                            padding=1)
        feat_dlr = F.relu(feat_dlr, inplace=True)
        feat_dlr = F.conv2d(feat_dlr,
                            torch.cat(
                                [self.kernel_shared_2, self.kernel_depth_2], dim=0),
                            torch.cat(
                                [self.bias_shared_2, self.bias_depth_2], dim=0),
                            padding=1)
        feat_dlr = F.relu(feat_dlr + shortCut, inplace=True)
        shortCut = feat_rgb
        feat_rgb = F.conv2d(feat_rgb,
                            torch.cat(
                                [self.kernel_shared_1, self.kernel_rgb_1], dim=0),
                            torch.cat(
                                [self.bias_shared_1, self.bias_rgb_1], dim=0),
                            padding=1)
        feat_rgb = F.relu(feat_rgb, inplace=True)
        feat_rgb = F.conv2d(feat_rgb,
                            torch.cat(
                                [self.kernel_shared_2, self.kernel_rgb_2], dim=0),
                            torch.cat(
                                [self.bias_shared_2, self.bias_rgb_2], dim=0),
                            padding=1)
...
class Coupled_Encoder(nn.Module):
    def __init__(self,
        ...
        self.init_deep = nn.Sequential(
            ...
            nn.Conv2d(dep_in_channels, n_feat, kernel_size=3, padding=1),
            nn.ReLU(True),
        )
        self.init_rgb = nn.Sequential(
            # in_channels, out_channels, kernel_size
            nn.Conv2d(guidance_in_channels, n_feat, kernel_size=3, padding=1),  # todo
            nn.ReLU(True),
        )
        self.coupled_feat_extractor = nn.ModuleList(
            [Coupled_Layer() for i in range(self.n_layer)])
...
class Decoder_Deep(nn.Module):
    def __init__(self,
        ...
        self.Decoder_Deep = nn.Sequential(
            ...
            nn.ReLU(True),
            # in_channels, out_channels, kernel_size
            nn.Conv2d(n_feats // 4, 1, kernel_size=3, padding=1),
            nn.ReLU(True),
        )
...
class DCTNet(BaseModule):
    ...
    def __init__(self, 
        ...
        self.Encoder_coupled = Coupled_Encoder(dep_in_channels,guidance_in_channels)
        self.Decoder_depth = Decoder_Deep()

    def get_K(self, H, W, dtype, device):
        pi = torch.acos(torch.Tensor([-1]))
        cos_row = torch.cos(pi * torch.linspace(0, H - 1,
                            H) / H).unsqueeze(1).expand(-1, W)
        cos_col = torch.cos(pi * torch.linspace(0, W - 1,
                            W) / W).unsqueeze(0).expand(H, -1)
    ...
    def forward(self, x, hr_guidance, **kwargs):
        ...
        high_Dim_D, high_Dim_R = self.Encoder_coupled(x, hr_guidance)
...
if __name__ == "__main__":
    ...
    hr_guidance = torch.randn((2, 3, 224, 224)).cuda()
    net = DCTNet(1,3,upscale_factor=2).cuda()
