            ...
            for start in range(0, self.delta + 1, self.g):
                ...
                x_list.append(
                    F.conv2d(
                        ...
                        weight1,
                        self.bias[
                            int(self.out_channels * (s * ch_ratio + start) / s_num / ch_ratio) : int(
...
class ConvBNReLU2D(torch.nn.Module):
    def __init__(
        ...
        if norm == "BN":
            ...
            self.norm = SwitchNorm2d(out_channels, using_moving_average=True, using_bn=True)
        elif norm == "Adaptive":
...
def torch_gaussian(channels, kernel_size=15, sigma=5):
    ...
    gaussian_kernel = (1.0 / (2.0 * math.pi * variance)) * torch.exp(
        (-torch.sum((xy_grid - mean) ** 2.0, dim=-1) / (2 * variance)).float()
    )
    ...
    gaussian_filter = nn.Conv2d(
        channels, channels, kernel_size=kernel_size, padding=(kernel_size - 1) // 2, groups=channels, bias=False
    )
...
class DownSample(nn.Module):
    def __init__(self, num_features, act, norm, scale=2):
        ...
        if scale == 1:
            self.layers = nn.Sequential(
                ConvBNReLU2D(
                    in_channels=num_features, out_channels=num_features, kernel_size=3, act=act, norm=norm, padding=1
                ),
                ConvBNReLU2D(
                    in_channels=num_features * scale * scale,
                    out_channels=num_features,
                    kernel_size=1,
                    act=act,
                    norm=norm,
...
class Head(nn.Module):
    def __init__(self, num_features, act, expand_ratio=1, guide_channels=3, in_channels=1):
        ...
        self.guide_head = nn.Sequential(
            ConvBNReLU2D(
                in_channels=guide_channels, out_channels=num_features * expand_ratio, kernel_size=3, padding=1
            ),
            ConvBNReLU2D(in_channels=num_features * expand_ratio, out_channels=num_features, kernel_size=1, act=act),
        )
...
class MeanShift(nn.Conv2d):
    def __init__(self, rgb_range, rgb_mean=0.3457, rgb_std=1.0, sign=-1):
