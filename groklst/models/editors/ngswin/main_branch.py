...
class ShallowExtractor(nn.Module):
    ...
    def flops(self, resolutions):
        ...
        flops = 0
        flops += (
            H * W * 3 * 3 * self.in_chans * self.out_chans + H * W * self.out_chans
        )  # conv1: conv.weight, conv.bias
        return flops
...
class PatchMerging(nn.Module):
    ...
    def flops(self, resolutions):
        ...
        flops += (H // 2) * (
            W // 2
        ) * 4 * self.dim * self.downsample_dim + self.downsample_dim  # linear.weight, linear.bias
        return flops
...
class EncoderLayer(nn.Module):
    ...
    def __init__(
        ...
        self.blocks = nn.Sequential(
            *[
                NSTB(
                    dim=dim,
                    training_num_patches=training_num_patches,
                    ngram=ngram,
                    num_heads=num_heads,
    ...
    def flops(self, resolutions):
        ...
        flops += (
            H * W * self.num_cas * self.dim * self.dim + self.dim
            if isinstance(self.across_cascade_proj, nn.Linear)
            else 0
        )
...
class SCDPBottleneck(nn.Module):
    ...
    def forward(self, shallow, x_list, num_patches_list):
        ...
        x_list = [
            pixel_shuffle_permute(x + self.bottleneck_pool(shallow, i), num_patches_list[i], num_patches_list[0])
            for i, x in enumerate(x_list)
        ]  # "S" pixel-Shuffle. upsample each input
    ...
    def flops(self, resolutions):
        ...
        # Depth-wise
        flops += (
            H * W * 3 * 3 * self.concat_dim + H * W * self.concat_dim + H * W * self.concat_dim
        )  # conv.weight, conv.bias, act
        # Point-wise
