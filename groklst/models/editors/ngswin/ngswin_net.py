...
from .etc_utils import denormalize

@MODELS.register_module()
class NGswin(BaseModule):
    """
    Args:
    ...
    def __init__(
        self,
        training_img_size=64,  # todo
        in_chans=3,  # todo
        target_mode="light_x2",  # todo
        ngrams=(2, 2, 2, 2),
        embed_dim=64,
        depths=(6, 4, 4),
        num_heads=(6, 4, 4),
        ...
        **kwargs,
        ...
        _assert(
            target_mode in ["light_x2", "light_x3", "light_x4"],
        ...
        dpr = [
            x.item() for x in torch.linspace(0, drop_path_rate, sum(depths) + sum(dec_depths))
        ...
        for i in range(self.num_encoder_stages):
            self.add_module(
                ...
                EncoderLayer(
                    dim=embed_dim,
                    training_num_patches=(self.training_img_size[0] // 2**i, self.training_img_size[1] // 2**i),
                    ngram=ngrams[i],
                    depth=depths[i],
        ...
        self.bottleneck = SCDPBottleneck(
            ...
            self.add_module(
                ...
                DecoderLayer(
                    ...
                    training_num_patches=(
                        self.training_img_size[0] // 2 ** (1 - i),
    ...
    def forward_encoder(self, x):
        ...
        e1_, e1, num_patches1 = self.encoder_layer1(
            c0, num_patches0
        )  # [B, HW, D], [B, HW/(2^2), D], (H/2, W/2). encoder-stage1

        c1 = torch.cat([self.inter_pool(c0, num_patches0), e1], dim=-1)  # [B, HW/(2^2), 2D], (H, W). ASPC
        e2_, e2, num_patches2 = self.encoder_layer2(
            c1, num_patches1
        )  # [B, HW/(2^2), D], [B, HW/(4^2), D], (H/4, W/4). encoder-stage2

        c2 = torch.cat([self.inter_pool(c1, num_patches1), e2], dim=-1)  # [B, HW/(4^2), 3D], (H, W). ASPC
        e3_, e3, num_patches3 = self.encoder_layer3(
            c2, num_patches2
        )  # [B, HW/(4^2), D], [B, HW/(8^2), D], (H/8, W/8). encoder-stage3
...
if __name__ == "__main__":
    ...
    net = NGswin(
        training_img_size=64,  # todo
        in_chans=1,  # todo
        target_mode="light_x2",  # todo ['light_x2', 'light_x3', 'light_x4']
    ).cuda()
    out = net(x)
    print(out.shape)
