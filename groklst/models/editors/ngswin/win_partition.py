...
class NGramContext(nn.Module):
    ...
    def __init__(self, dim, window_size, ngram, ngram_num_heads, padding_mode="seq_refl_win_pad"):
        ...
        _assert(
            padding_mode in ["seq_refl_win_pad", "zero_pad"],
    ...
    def seq_refl_win_pad(self, x, back=False):
        if self.ngram == 1:
            return x
        x = (
            TF.pad(x, (0, 0, self.ngram - 1, self.ngram - 1))
            if not back
            else TF.pad(x, (self.ngram - 1, self.ngram - 1, 0, 0))
        )
...
class NGramWindowPartition(nn.Module):
    ...
    def forward(self, x):
        ...
        windows = rearrange(
            x, "b (h wh) (w ww) c -> b h w wh ww c", wh=self.window_size, ww=self.window_size
        ).contiguous()  # [B, wh, ww, WH, WW, D]. semi window partitioning
        ...
        if self.shift_size > 0:
            ...
            )  # [B, ph, pw, D]. cyclic shift
            windows = rearrange(
                shifted_x, "b (h wh) (w ww) c -> b h w wh ww c", wh=self.window_size, ww=self.window_size
            ).contiguous()  # [B*wh*ww, WH, WW, D]. re-semi window partitioning

        windows = rearrange(
            windows, "b h w wh ww c -> (b h w) wh ww c"
        ).contiguous()  # [B*wh*ww, WH, WW, D]. window partitioning
